...
from underrail_translation_kit.msnrbf_parser.loaders import _load_system_class_value
...
class ClassValueLoaderTest(unittest.TestCase):
    ...
    @mock.patch("underrail_translation_kit.msnrbf_parser.loaders.load_system_class_with_members_and_types")
    def test_loading_class_with_id(self, mocked):
        mocked.return_value = SerializedObject(b"\x01\x02\x03")

        stream = BytesIO(b"\x04")
        result = _load_system_class_value(stream, {})
